...
- [x] T-003: tester – Test loop (Sonnet): save round-trip, PWA smoke (install + offline start), determinismus po loadu, benchmark report. Model: Sonnet.
- [x] T-004: reviewer – Review gate (Opus, právo re-run): DoD M0 komplet vč. benchmarku; nevyhovující benchmark → eskalace (Worker/cap), ne pokračování. Model: Opus.

## Quality Gates
- [x] Plan neobsahuje orchestratora jako agenta u žádného tasku
- [x] Implementace prošla test loop (save round-trip + PWA smoke)
- [x] Benchmark změřen PŘED potvrzením technického stropu capu
- [x] Review gate GO (= DoD M0)
...
## Carry-over do iter-006 (M1)
- BUG-001 (assertSerializable WeakSet) – oprava v M1.
- Reviewer SUGGESTION: SW self-precache cold-offline ověřit; save read-modify-write paralelní; htm licence Apache-2.0 oprava v impl/VENDOR.
- A2 tracked: závazné potvrzení capu 8h + D13 + PWA install/offline smoke na reálném low-end HW (mimo agentické prostředí).
