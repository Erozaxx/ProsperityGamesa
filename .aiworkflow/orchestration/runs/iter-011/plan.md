...
## Master Checklist
- [ ] T-001: architect – Detailní návrh (Opus) tasků iter-011: T1 klientský trh (D9/§9.1: marketState{goodsId:{available,max}}, cenová kubika, spread haggle 1.35/0.6, clamp available∈[0,max], buyGoods/sellGoods commands REGISTROVANÉ), T2 denní mean-reversion drift (k=0.2/den, marketDailyDrift), T3 getGoldValue(koš) jediné oceňovací API + market.inject kontrakt (negativní S-06 → POZITIVNÍ kontrakt), T4 karavany (sendCaravan command, cesty/návraty přes schedule), T5 UI market screen + karavany. Plus crime.js fix (přes resource vrstvu). Wiring + UI explicitně. Model: Opus.
