...
- [x] T-002: coder – Implementace (Sonnet) dle návrhu; tsc/test/grep + wiring + UI. Model: Sonnet.
- [x] T-003: tester – Test loop (Sonnet): formulas ceny vč. clampu, arbitrážní sanity (nákup→prodej není ziskový), drift, catch-up-safe, PLNÝ MVP e2e scénář (install→hraj→zavři→offline→vrať se→progres). Model: Sonnet.
- [x] T-004: reviewer – MVP GATE (Opus, právo re-run): ověření VŠECH acceptance criteria zadání; nesplnění = re-run. Model: Opus.

## Quality Gates
- [x] Plan neobsahuje orchestratora jako agenta u žádného tasku
- [x] Commandy (buyGoods/sellGoods/sendCaravan) registrované v runtime + market UI napojené
- [x] getGoldValue/market.inject kontrakty živé (S-06 pozitivní)
- [x] Implementace prošla test loop (arbitráž sanity + MVP e2e)
- [x] MVP GATE GO (všechna acceptance criteria zadání)
...
## MVP HOTOVÉ (MVP gate GO) — backlog mimo MVP
- S-1: qty input/karavana editor → M5; S-2: balance kalibrace (approximated) → M9; S-3: goods IDs vs design tabulka (zdokumentováno).
- M5–M9 kontrakty připravené (marketInject pro M7, getGoldValue pro tribute, caravan speed pro road techy).
