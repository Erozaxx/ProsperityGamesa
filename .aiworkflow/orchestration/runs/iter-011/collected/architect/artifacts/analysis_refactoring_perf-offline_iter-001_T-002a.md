...
**Návrhové rozhodnutí (G1, Med): seedovatelný/serializovatelný RNG.** Varianta (a) přehrává RNG mechaniky per krok; originál ale používá globální `Math.random()` (+ pomocný wrapper `services/rand.js`), který nelze seedovat ani uložit. Pro reprodukovatelný offline catch-up (stejný save + stejný zameškaný čas → stejný výsledek; laditelnost, replay) a pro testovatelnost vzorců (balanc-as-code, T-002b D2) musí rebuild použít **seedovatelný PRNG, jehož stav je součástí save**. Je to explicitní předpoklad varianty (a) – doplněno dle review T-003 (G1).
**Bitvy během catch-upu (G2, Med):** auto-resolve obranných bitev není zvláštní mechanika navíc – je přirozeným důsledkem návrhu bitvy jako **serializovatelného deterministického automatu na jednotném časovém zdroji** (A4 zde; T-002b C3; konsolidováno v review T-003 jako položka K7/K8): tatáž kroková funkce bitvy se při catch-upu jen spustí bez UI/hráčových commandů. Balanční/UX zbytek (cap, auto-resolve vs. odložení bitvy na návrat hráče) zůstává k eskalaci jako balanční otázka.
...
**Problém.** Tržní `basePrice/available/max` přicházejí z `GET /market` a přepisují lokální položky (market.js ř. 263–305). Volá se z `Market.step` s podmínkou `curStep % STEPSPERDAY * 5 == 0` (market.js **ř. 25** – precedence bug je v tomto volání, ne uvnitř `getUpdatedData`, která začíná až na ř. 263): `%` váže těsněji než `*`, takže `(curStep % 900) * 5 == 0` → fetch běží **každý herní den** (45 s reálného času), ne každých 5 dní. Error větev: při 401 **přesměruje hráče na `/signin`** (ř. 300–303). Bez dat: `calcMarketPrice` vrací 0 a loguje error (ř. 120–122).
...
| 15 | **B5-RNG** Seedovatelný/serializovatelný RNG místo `Math.random()` + `services/rand.js` | Med | S | Předpoklad reprodukovatelného catch-upu (#4) a testů vzorců; doplněno dle review T-003 (G1) |
