...
- [ ] T-004: coder – T1+T2 battle automat (L, determinismus-kritický): battleState (§8.1 kontrakt naplněn, serializovatelný string liege/lastAttackId – F-1), battleStep(bs,commands,rng('battle')) plný souboj (cd-down M-2 double-decrement 1:1, útoky warriors→archers, damage/crit M-3 rng pevný 1×/útok, revival M-1 baseRevival fallback z BALANCE, obranná AI auto-resolve G2 uvnitř battleStep), battleTick adaptér akumulátoru (subAccMs+=50, while≥30); damage/revival vzorce do formulas.js (battleDamage/Defense/revive tabulkové vs originál); combat staty do military.json (G-MILITARY-STATS approx). Persist state.battle round-trip + kill-resume test
- [ ] T-005: coder – T3: battleCommand commands (hráčské akce do battle queue) + registrace v bootstrapu (anti-dark-code); player AI vs opponent AI cesty v battleStep dle designu §7
