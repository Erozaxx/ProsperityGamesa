...
- [ ] T-002a: architect – Revize designu: zapracovat 4 major podmínky (M-1 effects→modifier mapování + eliminace dvojího započtení agregátů; M-2 sdílený rebuildBuildingDerived z load i complete/destroy; M-3 deterministické řazení foldu set; M-4 G-BUILD-TXAUDIT poznámka) + zúžit na M5-1 scope (T1–T4); minor/nit dle uvážení. Výstup: revidovaný design doc
- [ ] T-003: tom-proxy – Human gate: schválení revidovaného M5-1 designu (zastupuje uživatele dle DR-013-00)
- [ ] T-004: coder – T1: building instances (created/totalMade/instances), ageBuildings (day) opotřebení + opravy (getGoldValue), deklarativní persist schéma budov, `created===instances.length` re-derivace; balanc čísla do balance.js
