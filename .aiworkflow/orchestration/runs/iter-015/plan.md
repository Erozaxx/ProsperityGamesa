...
- [ ] T-005: coder – T2 dotažení/zatvrzení (implementace generalizace hotová v T-004): přidat KRITICKÝ round-trip identita test S odemčenými budovy-cílícími techy (buyTech → snapshot hashState → save → load → rebuild → bit-identický); ověřit determinismus persist/re-gen tech modifikátorů (save=jen unlockedTechs, tech modifikátory re-generovány na loadu identicky – ne dvojí započtení); catch-up-safe s techy; payload neobsahuje derivované tech data
