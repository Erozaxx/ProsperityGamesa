...
- [ ] T-001: architect – Detailní návrh (Opus) všech tasků iterace T1–T6 (struktura repa §3.1, state container §3.2, clock+akumulátor §4.1, scheduler heap+periodika §4.2, RNG streamy §4.4, tickOrder §4.3 + calendar/seasons + commands skeleton §3.3) → implementační spec pro Sonnet. Model: Opus.
- [ ] T-002: coder – Implementace (Sonnet) celé iterace dle návrhu T-001: src/ struktura, engine core, CI gate tsc+grep, package.json, node:test. Model: Sonnet.
- [ ] T-003: tester – Test loop (Sonnet/Haiku): tsc --checkJs zelené, node:test suite, determinism hash, grep gate, jednotkové testy clock/scheduler/RNG hran (den/sezóna/rok). Model: Sonnet.
