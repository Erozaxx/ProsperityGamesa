...
- [x] T-004: reviewer – Review gate (Opus, právo re-run): DoD iter-004, hranice vrstev, živé artefakty (tickOrder + diagram). Model: Opus.

## Quality Gates
- [x] Plan neobsahuje orchestratora jako agenta u žádného tasku
- [x] Implementace prošla test loop (tester) – tsc + node:test zelené
- [x] Review gate GO (reviewer Opus)
- [x] tickOrder + ASCII diagram existují jako živé artefakty
...
## Carry-over do iter-005
- SUGGESTION-1 (reviewer): přidat `.github/workflows/*.yml` pro CI gate při zavedení remote.
- BUG-001 (tech-debt): assertSerializable WeakSet ochrana – oprava v M1 (iter-006).
