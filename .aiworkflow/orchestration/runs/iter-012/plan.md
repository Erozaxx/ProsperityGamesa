...
- [ ] T-017: coder – Oprava F-1 + F-2 (schváleno uživatelem po T-011). F-1: healthBirths (health.js) + populationMigration (population.js) nesmí REDUKOVAT už-over-cap loaded total (guard `pop>=sanityCap ? pop : min(...)`) dle R-A4-3 (koherentně v OBOU systémech). F-2: použít exportovaný `populationSanityCap()` helper i v health.js (žádná inline duplikace). + regress test (loaded over-cap save: porod/migrace total neshrinkne). Gate: plné `npm run ci` zelené, smoke OK, determinismus G1 nedotčen
