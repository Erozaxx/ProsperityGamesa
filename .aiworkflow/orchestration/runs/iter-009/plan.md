...
- [x] T-004: reviewer – Review gate (Opus, právo re-run): pořadí uvnitř dne ověřeno proti zdroji a zapsáno do tickOrder (věrnost §4.3). Model: Opus.

## Quality Gates
- [x] Plan neobsahuje orchestratora jako agenta u žádného tasku
- [x] Implementace prošla test loop (produkce + catch-up-safe + workerEfficiency napojení)
- [x] tickOrder aktualizován (pořadí uvnitř dne věrné zdroji)
- [x] Review gate GO (= DoD M3)

## Exit Criteria
- produkční smyčka hratelná na mobilu; skilly progresují s kompenzací; vše catch-up-safe; reviewer GO.

## Carry-over do iter-010 (M4a) – nitpick/backlog z M3 review
- timeSinceLastFire inkrement; startSkill cost/discovery → M5+ gapy; UI progress-bar normalizace. (vše non-blocking)
