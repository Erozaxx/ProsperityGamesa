...
- **Task**: T-001, iter-016 (BRIEF-016-001)
- **Autor**: architect
- **Datum**: 2026-06-14
- **Milník**: M7a (AI svět: zóny + frakční AI + revolty/questy/tribute + jednotky + napojení trhu). Renumbering DR-013-00 (M7a = iter-016).
...
- **Round-robin přes 5denní periodu** (originál `step` ř.580–591, zdroj pravdy):
  ```
  period   = STEPSPERDAY * 5          // 5 herních dní; STEPSPERDAY=900 → period=4500
  dist     = ceil(period / zones.length)
  if (curStep % dist === 0 && zones.length > 0):
      zoneIndex = (curStep / dist) % zones.length
      processZone(state, zones[zoneIndex].id, rng)
  ```
  - **Determinismus**: `zoneIndex` je čistá funkce `curStep` a `zones.length` — **bezstavový round-robin**, žádné kurzorové pole v `state` (přežije save/load triviálně). Catch-up-safe: každý den se zpracuje 0–1 zóna, O(1) per tick.
  - **Gap G-WORLD-DAYEDGE**: originál pouští round-robin uvnitř `World.step` (každý krok), zde běží na **day edge** (worldTick order 30). Důsledek: `dist`/`zoneIndex` se počítají z `curStep` na denních hranách → výběr zóny řidší než v originálu (1 zóna / den max). Vědomá odchylka pro catch-up cenu (S-05). Kalibrace M9. **Coder**: `processZone` se volá max 1× za worldTick; round-robin přes periodu zachován.
...
### 7.5 Bezstavový round-robin
`zoneIndex = (curStep/dist) % zones.length` — **žádný kurzor v state** → save/load triviálně determinis­tický (na rozdíl od uloženého indexu, který by mohl driftovat). Při změně `zones.length` (revolt/takeover nemění počet zón, jen liege) zůstává stabilní.
...
5. round-robin v `worldTick` (bezstavový) + persist world.zones naplnění + re-hydratace load.
6. fresh-vs-load determinismus test + zone round-robin save/load test.
...
*Konec designu. Zdroj pravdy mechanik: originál `world.js`. Architektura: iter-002 §8.2/§9.1/§9.4 (D/K/R citováno průběžně). Renumbering: DR-013-00.*
