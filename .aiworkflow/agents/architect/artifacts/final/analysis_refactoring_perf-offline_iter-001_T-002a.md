...
**Problém.** `World.step` počítá `$rootScope.isNewDay/isNewNoon` z `Engine.curStep % STEPSPERDAY` (world.js ř. 568–569) – ale `Engine` (service) žádné `curStep` nemá (žije na `$rootScope.engine.curStep`) → `NaN % x` → flagy jsou **vždy false**. Cokoli na nich závisí, nikdy nenastane; služby si proto den/poledne počítají každá znovu lokálně (home.js ř. 601+).
...
**Problém.** `Game.save` ukládá jen přes `Gamesaves.$save/$update` („always save to server now!", game.js ř. 45); lokální záchrana `createLocalSave` je zakomentovaná (ř. 202–209, 231, 251) a error handlery save jsou **prázdné**. `localStorage` drží jen katalogový backup `itemlistbk` a drobnosti. Load jde rovněž jen ze serveru (`Gamesaves.get`, ř. 372).
