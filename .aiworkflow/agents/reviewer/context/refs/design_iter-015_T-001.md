...
- **Task**: T-001, iter-015 (BRIEF-015-001 / BRIEF-ID BRIEF-015-001)
- **Autor**: architect
- **Datum**: 2026-06-14
...
addTechModifiers(state):                       // čistá vůči RNG; čte unlockedTechs + catalog techs
  for techId in Object.keys(state.player.unlockedTechs) where unlockedTechs[techId]===true:
    tech = findTech(techId)                     // catalog, §4.2; pokud chybí → skip (defenzivně)
    for atom in (tech.effects ?? []):           // atom = {target, attr, op, value}
      mods.push({
        id: `tech:${techId}:${atom.attr}:${atom.op}`,   // POZN: pokud tech cílí 2× stejný (attr,op) na různé target → přidej target do id, viz níže
        source: `tech:${techId}`,
        target: atom.target,
        attr: atom.attr,
...
techs.json:
{
  ...
  "techs": {
    ...
    "tree": [
      ...
      { "id":"civil_planning", "sector":"civil", "level":0, "name":"Urban Planning",
        "prereqs":[], "provenance":"approximated",
        "effects":[ {"target":"house","attr":"attractiveness","op":"add","value":2} ] },
...
- **Cíle `target` musí existovat v katalogu** (job `farmer`/`baker`/`lumberjack`/`warrior`, budova `granary`/`house`) — coder ověří proti jobs.json/buildings.json/military.json; pokud cíl chybí, použij existující ID (reviewer gate: tech `target` ∈ známá ID). Efekt na neexistující ID = tichý no-op v `effective` (filtr nic nenajde), neblokuje, ale ladí se na M9.
